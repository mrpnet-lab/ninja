# Mm_ATM v7 — User Documentation

**Version:** 7.0.1 (2026-04-30) — forked from `Mm_ATM_v6` 4.2 stable
**Instrument:** NQ / MNQ futures (NinzaRenko 64/16 brick chart recommended)
**Platform:** NinjaTrader 8

> v7 preserves all v6 logic. Version-history comments inside the code are **kept as `v6 X.Y`** for forensic continuity. New work is tagged `v7 X.Y`.
...
**Status: ROLLED BACK to v7 0.3 same day. PnL was MOST NEGATIVE of any v7 release.** Code DELETED. Backup: `Old/Mm_ATM_v7_BKP_v1.0_FAILED_20260501.cs`. See `Mm_ATM_v7_spec.md` §21.3 for full root-cause analysis.
- **Lesson:** Entering on FIRST opposite-color brick in `TREND_*` is a trap-magnet (16% of all runs are 1-brick MM traps). Replacing v0.3 PxStop adaptive-retrace with a tight touch-exit chops winners on noise.
- **Active baseline restored:** v7 0.3 (`Old/Mm_ATM_v7_BKP_v0.3_2K_winner_20260501.cs`).

### v7 1.0 (2026-05-01) — "Reversal Rider" — original release notes (preserved for forensic)
**v7 0.4 and v7 0.5 were ROLLED BACK and DELETED** — they regressed PnL vs v7 0.3 ($2,250). v7 1.0 returns to the v7 0.3 base and adds 3 surgical patches under one master switch:
- **RR Entry** (`RR_ARMED`/`RR_FIRE`/`RR_CANCEL`) — on opposite-color brick close in `TREND_X`, arm a reversal candidate. Next bar, if color matches AND body ≥ `reversalConfirmBodyPct` (50%) of armed body, fire entry at market. Bypasses the streak/conf gates that caused late entries.
- **RR Tight Trail** (`RR_TIGHT_HIT`) — in trade in `TREND_X` with same-dir streak ≥ 2: trail = `prevBrickLow − 3tk` (LONG) / `prevBrickHigh + 3tk` (SHORT). Hidden SL ratcheted to trail itself. Touch-exit (no retrace gate, no min-peak gate).
- **RR Trap Re-entry** (`RR_TRAP_REENTRY`) — after RR_TIGHT exit, if 1-2 bricks later the same color resumes in matching TREND, re-enter at market (max `maxTrapReentries` = 2 per trend).
- New params: `reversalRiderEnabled` (true), `tightTrailBufferTicks` (3), `reversalConfirmBodyPct` (50), `maxTrapReentries` (2). All ON by default — no dead toggles.
- **HARD RULE:** if PnL < $2,250, revert to `Old/Mm_ATM_v7_BKP_v0.3_2K_winner_20260501.cs`.

### v7 0.3 (2026-04-30) — "Ride The Brick"
Validated against v7 0.2 ($1,275 / 9 trades). Solves over-trading inside single trends: 4/30 09:35 SHORT was sliced into 5 trades (+$520 net) when the underlying 17-brick R run was worth ~$1,600/contract. Root cause: `pxStopAdxFallingMult` (0.7x) tightened the trail's `effRetracePts` on a tiny ADX wobble (slope=-0.57) inside an alive 17-brick streak.
- **RIDE_LOCK in PxStop** — when alive same-direction streak ≥ `rideStreakMin` (5) AND last same-color brick within `rideRecentBrickSec` (30s) AND `lastAdxSlope > rideMinSlope` (-2.0): override falling-mult; apply `rideMult = min(1.5, 1 + 0.05 × (streak - min))`. Final `effRetracePts = max(adaptive-output, baseline × rideMult)`. Trail breathes wider as streak grows; never tightens during alive streak.
- New diag tag: `RIDE_LOCK_PXSTOP` (per fire, throttled per-bar). PREMIUM_BYPASS / CAP_REFUND / WOULD_TRADE all kept active.
- New params: `rideLockEnabled`, `rideStreakMin`, `rideMinSlope`, `rideStreakPerBrickBonus`, `rideStreakMaxMult`, `rideRecentBrickSec`.

### v7 0.2 (2026-04-30) — "Premium Re-Arm"
Fixes the lockout that v7 0.1's WOULD_TRADE audit exposed: 4/30 playback hit `MaxTradesPerDay=20` cap at 11:55 → missed a streak-43 TREND_UP run worth ~$3k/contract from 13:00–14:00.
- **PREMIUM_BYPASS** — when cap is hit, allow up to **5 extra entries** per session if **all** of: regime is TREND_UP/DN, brick color matches direction, streak ≥ `premiumStreak` (12), matching confidence ≥ `premiumMinConf` (85), `dailyRealizedPnL ≥ premiumMinPnl` ($0). Six-condition gate — only premium clean trends qualify.
- **CAP_REFUND on winners** — every closed trade with PnL ≥ `cdScratchThreshold` ($25) refunds one slot to `dailyTradeCount`. Stops "death by 20 tiny wins" lockouts.
- New diag tags: `PREMIUM_BYPASS`, `CAP_REFUND`. WOULD_TRADE audit kept active.
- New params: `premiumBypassEnabled`, `premiumStreak`, `premiumMinConf`, `premiumMinPnl`, `premiumExtraTrades`, `cdScratchThreshold`.

### v7 0.1 (2026-04-30) — v4.3 ports + audit instrumentation
- **Pre-RTH bypass** — AUTO entries allowed before RTH when regime is TREND_UP/DN AND brick streak ≥ `preRthTrendMinStreak` (12) AND brick color matches direction.
- **Streak-Hold loosen** — streak-hold flip-exit guard disabled when brick streak ≥ `streakHoldMaxStreak` (8). Clean impulse, legacy retrace catches reversal.
- **Chop peak-lock mult** — lifted 0.80 → 0.85 (UNKNOWN regime trades were over-tight in v4.2 playback).
- **WOULD_TRADE audit** — every silent BLOCK_AUTO with strong-signal context now also emits a `WOULD_TRADE` diag row. Lets us measure pre-RTH (and other) alpha BEFORE relaxing live gates.
- New diag tags: `PRE_RTH_BYPASS`, `WOULD_TRADE`.

Full change ledger: `Diags/Mm_ATM_v7_Changelog.md`.

---

## 1. What This Strategy Is

`Mm_ATM_v7` is an **anti-Market-Maker brick-trail strategy** for NQ.
...
`C:\Users\<you>\Documents\NinjaTrader 8\MmATM_v7_DiagLog_<date>.csv`
...
| `WOULD_TRADE,blockedBy=` | Audit row emitted with strong-signal context for any block (v7 0.1) |
| `PRE_RTH_BYPASS` | Pre-RTH outsideHours block bypassed by TREND+streak rule (v7 0.1) |
| `PREMIUM_BYPASS` | maxTrades cap bypassed by clean-trend rule (v7 0.2) |
| `CAP_REFUND` | Winner ≥ `cdScratchThreshold` refunded one slot to `dailyTradeCount` (v7 0.2) |
| `RIDE_LOCK_PXSTOP` | PxStop retrace widened by streak-aware ride bonus during alive same-dir streak (v7 0.3) |
| `RR_ARMED` | Reversal candidate armed on opposite-color brick close in TREND_X (v7 1.0) |
| `RR_FIRE` | Reversal entry fired (next bar confirmed color + body) (v7 1.0) |
| `RR_CANCEL` | Armed reversal cancelled (color or body confirm failed) (v7 1.0) |
| `RR_TIGHT_HIT` | RR tight-trail (1 brick behind, 3-tk buffer) touch-exit fired (v7 1.0) |
| `RR_TRAP_REENTRY` | After RR exit, same-color resumed in trend → re-entered (v7 1.0) |
...
| Version | Highlights |
|---------|-----------|
| **v7 1.0** | ❌ REVERTED — Reversal Rider regressed PnL (most negative). RR entry on first opposite brick = trap-magnet; tight touch-exit replaced PxStop = chopped on noise. Code DELETED. See spec §21.3. |
| **v7 0.3** | Ride The Brick: RIDE_LOCK_PXSTOP scales effRetraceMin by streak-aware mult (1.0–1.5x) so trail does not bail on small mid-streak retraces (🏆 active baseline) |
| **v7 0.2** | Premium Re-Arm: PREMIUM_BYPASS of maxTrades cap on TREND+streak≥12+conf≥85 (capped 5/session), CAP_REFUND on winners ≥ $25 |
| **v7 0.1** | Pre-RTH bypass on TREND+streak≥12, streak-hold loosen on streak≥8, chop peak-lock mult 0.80→0.85, WOULD_TRADE audit |
