...
## 26. Phase 3.0 - SHIPPED v6 3.0 + 3.1 (2026-04-30)

**Checkpoint backup:** `Old/Mm_ATM_v6_BKP_v2.12_PB19_1905win.cs` (PB19 baseline +$1,905 / 9W-1L, 90% win rate)

### 26.1 v6 3.0 - Three intelligent improvements

**A) Smart BE Loss-Cap Ladder** (replaces dumb `breakevenEnabled` BE which was stop-hunted by MM in v6 2.12)
- Ratchet-only, MM-invisible offsets (NOT round-number BE).
- Tier 1 LOSS-CAP: peak >= 4pt -> SL = entry - 6pt (caps disasters at -$120 NQ).
- Tier 2 SOFT-BE:  peak >= 6pt -> SL = entry + 0.75pt (3 ticks INTO profit, locks +$15 min).
- Tier 3 HANDOFF: peak >= 8pt -> BrickTrail PxStop owns the trail.
- Hands-off when: `!smartBeEnabled`, `runnerModeActive`, `breakevenLocked` (manual touch), `openDcaCount > 1`.
- Diag rows: `SMART_BE_LADDER`, `SMART_BE_HANDOFF`.
- Defaults: `smartBeTier1PeakPts=4`, `smartBeTier1MaxLossPts=6`, `smartBeTier2PeakPts=6`, `smartBeTier2LockPts=0.75`.
- Properties: Group `10 - Regime`, Order 70-74.

**B) Adaptive BrickTrail Retrace** (peak-scaled, replaces fixed 7.5pt min-retrace)
- `effRetracePts = clamp(peak * 0.30, [2.5, 7.5])`.
- Stacks before v6 2.9 ADX scaler; tight on small peaks (8-11pt winners no longer give back 30-71%), breathes for runners (peak>=25pt caps at 7.5pt).
- Defaults: `BrickTrailAdaptiveRetracePctOfPeak=0.30`, `Min=2.5`, `Max=7.5`.
- Properties: Group `10 - Regime`, Order 75-78.

**C) Regime-Quality Gate** (per-regime intraday PnL self-tightening, AUTO-only)
- Tracks PnL per regime (`TREND_UP`, `TREND_DN`, `CHOP`, `UNKNOWN`) in `regimePnlToday` dict.
- When a regime drops below `regimeQualityNegativeThresh` (default -$200), tightens entries for THAT regime only:
  - UNKNOWN: require `dvAtr >= 3.0` OR `trapScore > 0`.
  - TREND_*: require `htfBias` aligned with direction.
  - CHOP: blocks.
- Profitable regimes are NEVER restricted.
- Bypassed when `earlyFlip` or `brickReentryBypassActive`.
- Diag rows: `REGIME_PNL` (on every exit).
- Defaults: `regimeQualityGateEnabled=true`, `Thresh=-200`, `UnknownMinDistVwapAtr=3.0`.
- Properties: Group `10 - Regime`, Order 79-81.

### 26.2 v6 3.0 - Cleanup

- Removed ~120 lines of dead code (legacy `breakevenEnabled` SMART BE block - was disabled in v6 2.12 because MM stop-hunted the round-number entry+1tk SL).
- Trimmed verbose v6 3.0 explanatory comments to one-liners (full prose lives here in spec).

### 26.3 v6 3.1 - Manual-trader hardening

User trades this strategy MANUALLY in addition to AUTO. All manual controls must be production-stable.

**A) `NudgeSlPricePoints` (and tick variant) - manual lock on EVERY touch**
- Fix: ANY manual SL nudge (tighten OR widen) sets `breakevenLocked = true`.
- Reason: previously only tighten set the lock; widen could be undone by Smart BE Tier 2 ratchet (entry+0.75pt). Now manual = "I'm in charge; auto stays out".

**B) Tick-level manual precision (STEP toggle)**
- New dashboard button `STEP PT / STEP TK` (next to RUN, in `rowSmart`).
- `STEP PT` (default): SL/TP buttons step by `slTpAdjustStep` pts (default 5pt = 20tk = $100); Trail buttons step by `trailNudgeStepPoints` pts (default 1pt).
- `STEP TK`: SL buttons = +/- 1 tick precision (via new `RequestSlNudgeTicks` -> `NudgeSlPriceTicks`); Trail buttons = +/- 1 tick (via new `RequestTrailNudgeTicks` -> `NudgeTrailDistanceTicks`); TP buttons = +/- 1 pt (smallest possible since `tpPoints` is integer).
- Internal API: `pendingSlNudgeTicks` and `pendingTrailNudgeTicks` accumulators dispatched in `ProcessPendingButtons`.
- `NudgeSlPriceTicks` is the new canonical SL nudge; `NudgeSlPricePoints(dPts)` now delegates as `dPts * 4` ticks.
- `NudgeTrailDistanceTicks` mirrors `NudgeTrailDistancePoints` but uses `dTicks/4.0` for the offset.
- Property: `ManualTickMode` (Group `3 - Trail / SL`, Order 8) - persists default across template loads.
- Diag row: `TOGGLE_STEP TK|PT` on every flip.

**C) Manual control safety matrix (verified)**

| Control | Function | Sets `breakevenLocked` | Smart BE hands-off | Ratchet-safe |
|---------|----------|:----------------------:|:------------------:|:------------:|
| Jump SL    | `ExecuteJumpSL`         | YES | YES | YES |
| SL +/- (pt) | `NudgeSlPricePoints`    | **YES (v6 3.1 - both directions)** | YES | YES |
| SL +/- (tk) | `NudgeSlPriceTicks`     | YES | YES | YES |
| Trail +/- (pt) | `NudgeTrailDistancePoints` | n/a (separate path) | n/a | YES (tighten always; loosen only if not ratcheted past) |
| Trail +/- (tk) | `NudgeTrailDistanceTicks`  | n/a | n/a | YES |
| TP +/- | inline `tpPoints +/-`   | n/a | n/a | n/a (TP can move freely) |
| TRL NOW | `RequestTrailActivate`  | n/a | n/a | YES |
| RUN ON  | `runnerModeActive`      | n/a | YES (Smart BE skipped) | YES |

### 26.4 Validation plan

Run Playback session (PB20+) with v6 3.1 and verify:
1. `SMART_BE_LADDER` rows fire at peaks >=4pt / >=6pt with correct SL targets.
2. `SMART_BE_HANDOFF` appears at peak >=8pt and BrickTrail PxStop takes over.
3. `effRetracePts` shrinks on small-peak winners (visible in PXSTOP diag rows).
4. `REGIME_PNL` rows accumulate per regime; UNKNOWN entries get tightened only after losses.
5. Manual SL widen mid-trade -> Smart BE never re-tightens it (verify `BE_LOCK` flag in `SL_NUDGE` row).
6. STEP TK button -> SL +/- moves by exactly 1 tick (0.25 NQ pt = $5).
7. Multi-day regression: replay 3-5 days mixing trend/chop/low-ADX; PB19 day must still finish >=+$1,500 (no regression from +$1,905 baseline).

### 26.5 Deferred

- TP true tick-precision (requires refactoring `tpPoints` from int to double).
- v6 3.2 - Re-Entry on Same Trend (deferred from Phase 2.8).
