# Mm_ATM_v6 — Design Specification & Implementation Summary

**Version:** 6.0  
**Date:** April 28, 2026  
**Base reference:** `Mm_ATM_v5.cs` v5 14.20 (frozen — kept intact for live trading and A/B comparison)  
**Target file:** `Mm_ATM_v6.cs`  
**Platform:** NinjaTrader 8.1+ — NQ Futures (MNQ compatible)

> **Note on lineage:** Historical sections below (the v4 → v5 rewrite, the original 12-filter design, etc.) are preserved as-is for context. They describe how the codebase reached its v5 14.20 baseline, which is the seed `Mm_ATM_v6.cs` was forked from. New v6 work is documented in §18 (v6 roadmap) and §19 (active workplan), and all *new* version tags use `v6 0.x` numbering.
...
|---|---|---|
| Class rename to `Mm_ATM_v6` | ✅ | All `[Mm-ATM v6]` tags, Name = "Mm_ATM_v6" |
...
```
┌─────────────────────────────────────┐
│  ≡  NQ  Mm-ATM v6             ↙↘  │  ← draggable title + resize grip
├─────────────────────────────────────┤
...
Every entry attempt (auto OR manual button) flows through `CanEnterTrade(label, isManual, direction)` in `Mm_ATM_v6.cs` ~line 1823. If any blocker fires, the dashboard shows **`<label> blocked: <reason>`** in orange/red and (where noted) writes a `BLOCK_*` row to the diagnostic CSV. Order matters — first match wins.
...
### 19.2 Item 2 — Drag SL/TP/Trail lines on chart (v6 14.22)
...
### 19.3 Item 3 — Buy/Sell Limit ±N tick offset (v6 14.23)
...
### 19.4 Item 4 — AGGR-L1 / AGGR-L2 + PACE indicator (v6 14.24)
...
### 19.5 Item 5 — Lead-signal entry (v6 14.25)
...
### 19.6 Item 6 — NinjaRenko follow mode (v6 14.26 — biggest)
...
| v6 0.2.0 | 2026-04-28 | Phase 0.2 — NinzaRenko 64/16 secondary series plumbing | (1) `AddDataSeries(BarsPeriodType.Renko, Value=64, Value2=16)` adds `BarsArray[2]` in `OnStateChange.Configure`. (2) New `ProcessRenkoBar()` runs in `BarsInProgress==2`, tracks `lastBrickColor` (G/R) and `brickStreakCount` from prior closed brick (`Closes[2][1]` vs `Opens[2][1]`). (3) Diag log `BrickColor`/`BrickStreak` columns now show real values; new `BRICK_CLOSE` verb logs every brick with `o/c/hi/lo`. (4) `WriteDiagRow` hardened to use explicit primary-series indices (`Closes[0][0]`, `Times[0][0]`, `CurrentBars[0]`) — safe to call from any BIP. (5) New properties group **9 - Renko**: `EnableRenkoSeries` (default ON), `RenkoBrickSize` (64), `RenkoBrickOffset` (16). Defensive: legacy templates loading with `<4`/`<0` are auto-corrected. **No entry/exit logic uses Renko yet** — that ships in Phase 2.1 (W6). |
| v6 0.2.1 | 2026-04-28 | Phase 0.2 polish — read-only brick status row on dashboard | New `lblBrickInfo` between Hours and Quick-Action buttons. Reads `lastBrickColor` + `brickStreakCount` snapshots already maintained by `ProcessRenkoBar` — zero new computation. Format: `Brick: GREEN ×3  (64/16)`, color matches brick (LimeGreen/OrangeRed/Gray). Shows `Brick: off` when `EnableRenkoSeries=false`. Renko panel-properties intentionally NOT exposed as dashboard buttons because `AddDataSeries` is locked at `State.Configure` (a runtime change would silently no-op or force a reload that kills the open position). The runtime `RNK: OFF/ON/THRUST` mode toggle ships with W6 in Phase 2.1. |
| v6 0.2.2 | 2026-04-28 | Phase 0.2 fix — Renko brick processor reliability | User reported brick row stuck on gray "Brick: —" even with `EnableRenkoSeries=true` and visibly green Renko bricks. Root cause: `IsFirstTickOfBar` on a secondary series under `Calculate.OnBarClose` is unreliable — can stay false on the first bar after each strategy enable, and may not retrigger consistently. Fix: replaced `IsFirstTickOfBar` gate with explicit `lastProcessedRenkoBar` int dedupe (skip if `CurrentBars[2]==lastSeen`). Now processes every new brick regardless of tick-mode quirks. Also added one-shot `Print(TAG + "Renko series ALIVE …")` on first brick so user can confirm in NT8 Output window that `BarsArray[2]` is wired. Hardened color streak: only increments when prior color was non-empty (previously `""==""` would inflate streak across blank starts). |
...
> **Status (Apr 27, 2026):** v6 14.19 is the **stable production line**. No more invasive changes go into v5 — only bug fixes and parameter tuning. All structural ideas below are deferred to **`Mm_ATM_v6.cs`** (new file, fork of v6 14.19).
...
- **Fork**: copy `Mm_ATM_v6.cs` → `Mm_ATM_v6.cs`, rename class, bump display name.
...
1. **Freeze v6 14.19** — only bug-fix commits (e.g. `v6 14.19.1`) for production.
2. Open `ninja-v6` branch off current `ninja`.
3. Fork file, split into partials, get it compiling identical to v6 14.19 → tag `v6 0.1 - parity`.
...
> **Source data:** `MmATM_v6_DiagLog_20260428.csv` + two NinzaRenko 64-tick / 16-offset chart screenshots (one wide-angle showing the ~400-pt overnight downtrend, one zoom showing the 9:45 AM → 12:50 PM range).
